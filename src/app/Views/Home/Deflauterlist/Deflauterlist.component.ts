...
@Component({
    selector: 'app-home',
  templateUrl: './Deflauterlist.component.html',
  styleUrls: ['./Deflauterlist.component.css'],
    standalone: false
})

export class DeflauterlistComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.route.queryParams.subscribe(async params => {
      ...
      this.searchModel.OrderPassedby = "0";
      await this.GetVistorCount("InsertVistorCount");
      await this.GetVistorCount("GETCOUNT");
      await this.GetDeflauterlist();
  ...
  async GetDeflauterlist() {
    try {
      debugger
      await this.HomeService.GetDeflauterlist(this.searchModel)
        .then((data: any) => {
          data = JSON.parse(JSON.stringify(data));
          //console.log(data);
          //this.Deflauterlist = data['Data'];
          this.Deflauterlist = data['Data'] || [];
          //console.log(this.Deflauterlist, "Deflauterlist")
  ...
  viewProject(project: any) {
    this.routers.navigate(['/ViewProject'], {
      queryParams: {
        id: project.EncryptedProjectId,
        type: 'U'
      }
    });
  }
  ...
  calculatePagination() {
    this.totalPages = Math.ceil(this.Deflauterlist.length / this.pageSize);
  ...
  updatePagedData() {
    ...
    this.pagedProjectList = this.Deflauterlist.slice(startIndex, endIndex);
    //this.cdr.markForCheck();
  ...
  get startRecord(): number {
    if (this.Deflauterlist.length === 0) return 0;
