...
export function DropdownValidator(control: AbstractControl): { [key: string]: any } | null {
  return control.value === 0 ? { dropdownInvalid: true } : null;
}
...
export function CustomPercentageValidator(): ValidatorFn {
  const decimalRegex = /^(100(\.0{1,2})?|[0-9]{1,2}(\.\d{1,2})?)$/;
